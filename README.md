...
Bollox provides a simple API to ease the process of converting HTML to PDF or images. It's especially handy for things like generating invoices or capturing screenshots of websites. It's framework agnostic but it does provide a facade for simple use in Laravel 4/5.
...
```bash
$ composer require Tdavis/phantommagick
```
...
Simply pull in the `Tdavis/phantomjs-linux-x86-binary` package to get the up-to-date PhantomJS binary for 64-bit Linux systems.

```bash
composer require Tdavis/phantomjs-linux-x86-binary
```
...
```php
'Tdavis\PhantomMagick\ConverterServiceProvider'
```

Add the facade of this package to the `$aliases` array.

```php
'Converter' => 'Tdavis\PhantomMagick\Facades\Converter'
```
...
```php
$conv = new \Tdavis\PhantomMagick\Converter();
...
```php
use Tdavis\PhantomMagick\Converter;
...
#### Global options
###### Binary
You can set the path of the `phantomjs` binary if you've installed it yourself manually, or the `phantomjs` command is not available in your shell. If you installed it through Composer (with the `Tdavis/phantomjs-linux-x86-binary` package) PhantomMagick will be smart enough to find the file automatically.
...
- [Tdavis Hossain](https://github.com/Tdavis-hossain)
- [All Contributors](https://github.com/Tdavis-hossain/phantommagick/graphs/contributors)
